...
%define used_libs libocci.so.18.1 libclntsh.so.18.1 libnnz18.so libociei.so libocijdbc18.so
...
for i in %used_libs ; do
	/usr/sbin/semanage fcontext -a -t textrel_shlib_t '/usr/lib/oracle/18\.5/client.*/lib/'${i//./\\.}
done
...
/usr/sbin/semanage fcontext -a -t oracle_sqlplus_exec_t '/usr/lib/oracle/18\.5/client.*/bin/sqlplus'
/usr/sbin/semanage fcontext -a -t textrel_shlib_t '/usr/lib/oracle/18\.5/client.*/lib/libsqlplus\.so'
/usr/sbin/semanage fcontext -a -t textrel_shlib_t '/usr/lib/oracle/18\.5/client.*/lib/libsqlplusic\.so'
%{sbinpath}/restorecon -Rvv /usr/lib/oracle/18.5/client* || :
...
if [ $1 -eq 0 ]; then
	for i in %used_libs ; do
		/usr/sbin/semanage fcontext -d -t textrel_shlib_t '/usr/lib/oracle/18\.5/client.*/lib/'${i//./\\.}
	done
	%{sbinpath}/restorecon -Rvv /usr/lib/oracle/18.5/client* || :
fi
...
if [ $1 -eq 0 ]; then
	/usr/sbin/semanage fcontext -d -t oracle_sqlplus_exec_t '/usr/lib/oracle/18\.5/client.*/bin/sqlplus'
	/usr/sbin/semanage fcontext -d -t textrel_shlib_t '/usr/lib/oracle/18\.5/client.*/lib/libsqlplus\.so'
	/usr/sbin/semanage fcontext -d -t textrel_shlib_t '/usr/lib/oracle/18\.5/client.*/lib/libsqlplusic\.so'
	%{sbinpath}/restorecon -Rvv /usr/lib/oracle/18.5/client* || :
fi
